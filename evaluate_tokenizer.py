...
def evaluate_tokenizer(config_path, model_type, data_path, output_path):
    ...
    elif model_type == 'siglipvae':
        from tokenizer.siglipvae import SIGLIP_VAE
        model = SIGLIP_VAE(config_path).load().model
    ...
    folder_name = {
        ...
        'siglipvae': os.path.splitext(os.path.basename(config_path))[0],
        'sdvae': 'sdvae',
        'marvae': 'marvae'
    }[model_type]
    ...
    for batch in val_dataloader:
        ...
        with torch.no_grad():
            decoded_images_tensor = {
                ...
                'siglipvae': lambda: model.decode(latents),
...
def encode_images(model, images, model_type='vavae'):
    with torch.no_grad():
        posterior = {
            ...
            'siglipvae': lambda: model.encode(images),
            'marvae': lambda: model.encode(images),
            'sdvae': lambda: model.encode(images).latent_dist
        }[model_type]()
        if model_type == 'siglipvae':
            z = posterior.sample()
            aux_feature = model.foundation_model(images)
            aux_feature_proj = model.linear_proj_cat(aux_feature)
            z_add_aux = z + aux_feature_proj
            return z_add_aux.to(torch.float32)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_path', type=str, default='tokenizer/configs/siglipvae_f16d32.yaml')
    parser.add_argument('--model_type', type=str, default='siglipvae')
