...
class AutoencoderKL_SV(nn.Module):
    def __init__(self, embed_dim, ch_mult, use_variational=True, ckpt_path=None, model_type='siglipvae'):
        super().__init__()
        self.encoder = Encoder(ch_mult=ch_mult, z_channels=embed_dim)
        if model_type == 'siglipvae':
            self.decoder = Decoder(ch_mult=ch_mult, z_channels=embed_dim, attn_resolutions=(16,))
        elif model_type == 'marvae':
            self.decoder = Decoder(ch_mult=ch_mult, z_channels=embed_dim, attn_resolutions=())
        self.use_variational = use_variational
        ...
        print(f"Using {model_type} as auxiliary feature.")
        self.foundation_model = aux_foundation_model('siglip')

        # print('foudation_weight before:', self.foundation_model.model.vision_model.encoder.layers[0].self_attn.k_proj.weight)
    ...
    def init_from_ckpt(self, path):
        if self.model_type == 'siglipvae':
            sd = torch.load(path, map_location="cpu")
            # print('sd.keys():', sd.keys())
            if 'state_dict' in sd.keys():
                sd = sd['state_dict']

            # print('foudation_weight after:', sd['foundation_model.model.vision_model.encoder.layers.0.self_attn.k_proj.weight'])
            sd = {k: v for k, v in sd.items() if 'loss' not in k}
            msg = self.load_state_dict(sd, strict=False)
        elif self.model_type == 'marvae':
            sd = torch.load(path, map_location="cpu")["model"]
            msg = self.load_state_dict(sd, strict=False)
