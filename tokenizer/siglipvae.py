...
class SIGLIP_VAE:
    ...
    def load(self):
        ...
        self.model = AutoencoderKL_SV(
            embed_dim=self.embed_dim,
            ch_mult=(1, 1, 2, 2, 4),
            ckpt_path=self.ckpt_path,
            model_type='siglipvae'
        ).cuda().eval()
        return self
    ...
    def encode_images(self, images):
        ...
        with torch.no_grad():
            ...
            z = posterior.sample()
            aux_feature = self.model.foundation_model(images.cuda())
            aux_feature_proj = self.model.linear_proj_cat(aux_feature)
...
if __name__ == "__main__":
    vae = SIGLIP_VAE('tokenizer/configs/vavae_f16d32_vfdinov2.yaml')
    vae.load()